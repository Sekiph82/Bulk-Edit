...
Post-launch production QA. Production is **LIVE** under Private Beta (new sign-ups paused) since 2026-07-06. **Etsy OAuth is fully live and confirmed working end-to-end**: `sekiphayit1982@gmail.com` (superuser, `pro_monthly` comp grant) has shop WearYourStoriesCom (44263504) connected with all 210 active listings synced. All planned sprints (0-27) are complete. PR #93 (merge `1c27337`) deployed 2026-08-28; owner confirmed **title write now succeeds live**, price write still `HTTP 400`. Owner then requested internal-evidence-only diagnosis (VPN IP concern, no Etsy calls from Claude) — production logs confirmed the endpoint path is now correct and the failure is live/current-code. Current work: **fetch-patch-put inventory refactor** (branch `fix/internal-price-test-record-audit`, issue #92) — code-complete and test-verified as of 2026-08-28, pending PR/CI/merge/deploy — see `HANDOFF.md` for exact resume steps.
...
| Bulk Edit price write — fetch-patch-put refactor | **Code-complete, not yet merged (2026-08-28)** — branch `fix/internal-price-test-record-audit`, issue #92. Internal-evidence-only diagnosis (owner on VPN, no Etsy calls performed): production logs confirmed the failed request (`PUT .../listings/1874506717/inventory → HTTP 400`, apply_job_id `c23322f0-...`) ran against current code with the correct endpoint path. Implemented `apply_single_listing_price_quantity()` — GETs Etsy's live inventory tree, mutates only the changed price/quantity field, PUTs the full tree back, preserving `product_id`/`offering_id` the old minimal-payload builder couldn't. No live Etsy write performed. |
...
- Backend tests: `test_bulk_edit_inventory.py` 31/31 passed on `fix/internal-price-test-record-audit` (2026-08-28). `test_bulk_edit_variation.py`/`test_bulk_edit_apply.py`/`test_bulk_edit_revert.py`/`test_bulk_edit.py`: 133 passed, 6 pre-existing 401-vs-403 baseline failures (no regressions). No frontend files changed this round, so frontend checks weren't re-run (last verified clean on PR #93).
...
- **Fetch-patch-put refactor not yet merged/deployed** — code-complete on `fix/internal-price-test-record-audit`, needs PR open → CI green → merge → prod deploy → owner-approved controlled live retest. See `HANDOFF.md`.
- **Bulk Edit price write still not proven against a live Etsy write** — this is the *fourth* fix attempt on the write path (PR #89's `property_values` fix, PR #91's inventory-URL fix, PR #93's title shop-scope + inventory schema-keys fixes, and this round's fetch-patch-put refactor). Title write is confirmed working live (owner-verified after PR #93). Price write: each round diagnosed from code/logs + this codebase's own already-correct sibling patterns, mocked-test coverage only, no live Etsy calls performed by Claude any round. This round's fix is the strongest evidence yet (log-confirmed correct URL, code-level gap now closed) but still needs an owner-approved controlled single-listing live retest to confirm.
