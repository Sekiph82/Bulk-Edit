...
Post-launch production QA. Production is **LIVE** under Private Beta (new sign-ups paused) since 2026-07-06. **Etsy OAuth is fully live and confirmed working end-to-end**: `sekiphayit1982@gmail.com` (superuser, `pro_monthly` comp grant) has shop WearYourStoriesCom (44263504) connected with all 210 active listings synced. All planned sprints (0-27) are complete. **PR #100 (readiness_state_id fix) merged and deployed 2026-08-28 — the owner's live retest succeeded**: French Bulldog listing price 6000→6288, Etsy Shop Manager and Bulk Edit both reflected the new price. The multi-round Bulk Edit price-write bug (payload shape, endpoint scoping, readiness_state_id) is resolved. A subsequent live apply on a different listing hit `HTTP 429 "Exceeded per second rate limit"` — expected, since no Etsy write call had retry/backoff or inter-item pacing before this. Current work: **Etsy rate-limit guard/backoff** (branch `fix/etsy-rate-limit-guard`, engineering-only, based on `origin/main`, not the open docs PR #101) — code-complete and test-verified as of 2026-08-28, pending PR/CI/merge/deploy — see `HANDOFF.md` for exact resume steps.
