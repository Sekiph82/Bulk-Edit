...
| Etsy developer app | **Credentials received from Etsy 2026-07-31** (`bulk-edit-app`, rate limit 5 QPS / 5000 QPD — matches existing `ETSY_API_REQUESTS_PER_SECOND`/`ETSY_API_DAILY_LIMIT` defaults, no code change needed) and configured on `bulk-edit-prod-api` as encrypted `SECRET` env vars. Production OAuth URL generation verified (masked keystring `qvmj...fh33`, callback/scopes/PKCE all correct). **Root cause found (2026-08-27, via production logs after `doctl` re-auth):** token exchange succeeds, but `GET /v3/application/users/{user_id}/shops` returns **403** — most likely the app's Personal Use access tier doesn't cover this endpoint (tracked in issue #80; `user_id` derivation itself confirmed correct against Etsy's docs). Defensive `user_id` validation shipped as hardening (does not fix the 403). No shop connected yet (`total_shops=0`). **Do not retry OAuth until issue #80 is resolved** — a blind retry will very likely reproduce the same 403. |
