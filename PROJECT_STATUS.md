...
| Bulk Edit price write — readiness_state_id required | **Merged and deployed (2026-08-28)** — PR #100, merge `c880c91`. **Owner's live retest succeeded** — French Bulldog listing, Success 1/Failed 0/Skipped 0, confirmed on Etsy (`$62.88`) and in-app (`USD 62.88`). The price-write payload/schema problem is resolved. Magic Revert on this same apply job is the next owner-run proof needed (pending). A separate follow-up test hit `HTTP 429` (Etsy per-second rate limit) — tracked as a new Sprint 2 risk in `TASKS.md`, not a recurrence of the payload bug. |
...
- **Magic Revert not yet owner-verified** — the successful French Bulldog price apply job has not yet been reverted live. This is the immediate next owner-run test (Sprint 1, see `TASKS.md`).
- **Etsy per-second rate limit (`HTTP 429`)** — discovered on a follow-up manual price test. Do not run 3/10/33-listing batch write tests until a rate-limit guard/backoff exists (Sprint 2) or the owner explicitly accepts the risk for a slow manual test. See `TASKS.md` Sprint 2.
