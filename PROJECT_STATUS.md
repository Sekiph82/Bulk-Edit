...
| Etsy developer app | **Credentials received from Etsy 2026-07-31**, configured on `bulk-edit-prod-api` as encrypted `SECRET` env vars. **Etsy OAuth shop connection confirmed working end-to-end in production, 2026-08-27.** Connected shop: **WearYourStoriesCom**, Etsy shop ID `44263504`, `is_connected=True`, `last_synced_at=None` (no sync run yet, no writes performed). Issue #80 closed. Fixed in sequence to get here: Private Beta callback masking, missing categorized logging, `x-api-key` header missing the shared secret (merged `9336c53`), and `getShopByOwnerUserId`'s single-object response parsed as a list (merged `fee6e4c`). Next step: owner-approved controlled read-only listing sync validation (not started). |
...
- **Etsy shop connected, listing sync not yet performed.** Live OAuth completed successfully 2026-08-27 (WearYourStoriesCom, shop ID `44263504`) — needs explicit owner approval per `TASKS.md` → Owner Action before the read-only listing sync, live reads, or the never-tested-live video-upload endpoint can be exercised.
...
**Await owner approval for a controlled, read-only listing sync validation** (shop is connected but never synced). Do not create a new Etsy developer app, do not disable Private Beta, do not enable Etsy-derived external AI processing, do not perform any Etsy write, and do not submit another appeal.
