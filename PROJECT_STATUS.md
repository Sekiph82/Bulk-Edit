...
| Etsy developer app | **Credentials received from Etsy 2026-07-31** (`bulk-edit-app`, rate limit 5 QPS / 5000 QPD — matches existing `ETSY_API_REQUESTS_PER_SECOND`/`ETSY_API_DAILY_LIMIT` defaults, no code change needed) and configured on `bulk-edit-prod-api` as encrypted `SECRET` env vars. Production OAuth URL generation verified (masked keystring `qvmj...fh33`, callback/scopes/PKCE all correct). **Two live OAuth attempts made (2026-08-27)**, both reached Etsy and returned to `/shops` with "Failed to connect Etsy shop" — root cause still uncategorized at the log level (`fix/etsy-oauth-safe-callback-logging` adds safe categorized logging for the next attempt; DO log access itself is separately blocked by an expired `doctl` auth token, owner action needed). No shop connected yet (`total_shops=0` confirmed via `/etsy/shops`). |
