...
| Etsy developer app | **Credentials received from Etsy 2026-07-31** (`bulk-edit-app`, rate limit 5 QPS / 5000 QPD) and configured on `bulk-edit-prod-api` as encrypted `SECRET` env vars — both `ETSY_CLIENT_ID` and `ETSY_CLIENT_SECRET` were configured that session, but `ETSY_CLIENT_SECRET` was never wired into application code until 2026-08-27. Production OAuth URL generation verified (masked keystring `qvmj...fh33`, callback/scopes/PKCE all correct). **Stronger root-cause candidate found (2026-08-27):** token exchange succeeds, but `GET /v3/application/users/{user_id}/shops` returned **403** — traced to every Etsy v3 API call in the codebase sending `x-api-key: <keystring>` instead of Etsy's documented `<keystring>:<shared_secret>` format. Fix (`fix/etsy-oauth-shop-lookup-x-api-key`, **PR open, not yet merged**) wires up the already-configured `ETSY_CLIENT_SECRET` — no new production secret needed. Superseded, but kept as fallback: the earlier Personal Use access-tier hypothesis (issue #80) if the header fix doesn't resolve it on the next attempt. No shop connected yet (`total_shops=0`). **Do not retry OAuth until this PR is merged and deployed.** |
