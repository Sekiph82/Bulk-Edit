...
Phase: Private Beta production QA. M00-M09 PASS/CLOSED-with-known-gaps; M10 (Listing Health/Insights detail), M03.04 (shared ListingPicker), M13/M15 (media/variation read-only depth), M16 (Magic Revert history/plan-gate), and M19 (beta readiness matrix/runbooks) all SHIPPED and merged (PRs #107–#114) as of 2026-08-30. Owner-QA-polish (4 items) merged as PR #115 (`e2bfe46`). Owner completed both recommended live production write tests — title write + Magic Revert (OK), price write + Magic Revert (OK). Dashboard onboarding tracking bug + `[object Object]` display bug fixed and merged as PR #116 (`2ec4226c`). Account profile name fields + sidebar identity cleanup + small beta-readiness polish merged as PR #117 (`96b9f027`). Full `TASKS.md` truth audit merged as PR #119 (`77e1b250`) — corrected a badly stale "Milestone policy" summary, added CSV Import/Export (M03.06) and Scheduled Jobs (M04.06) which had zero tracking, upgraded M08.04/M12.02/M14 from "not started" to `[~]`. **Current work: M03.02 full-status listing sync + M03.03 Listings filters/counts, and Dashboard card removal** (branch `feature/m03-full-status-sync-and-listing-filters`) — owner rejected the "Owner-verified production checks" Dashboard card, removed (facts stay in docs only); `sync_shop_listings()` now fetches all 5 Etsy listing states read-only (`sold_out` native, not derived); new Sold out tab + real per-status counts on `/listings`. Both `[~]` — mocked-test-verified, no production sync run this round.
...
Implemented and tested but NOT owner-click-through-verified (code+test evidence only — see `TASKS.md`'s "Current Truth Snapshot" for the full list): owner admin console (`/owner`, M08.04), AI listing suggestions (`/ai`, M12.02), Dynamic Pricing + Profit Calculator (`/pricing-rules`, `/profit`, M14), CSV Import/Export (`/csv`, M03.06), Scheduled Jobs (`/scheduled`, M04.06), variation price/quantity preview (M15.02/M15.03), variation live apply (M15.04), full-status listing sync (M03.02), Listings status filters/counts (M03.03) — M03.02 additionally has no production-sync verification at all yet, mocked tests only.
Current manual proof needed: variation apply/revert never owner-live-verified (separate, optional, no revert exists yet — M15.04); media upload/delete/replace remains disabled pending a restore endpoint (M13.04); video generation never run; a real (owner-approved) production sync to confirm M03.02 against live Etsy data; broader beta launch, real Stripe live-billing readiness, and wider beta user onboarding remain untouched/pending. See `TASKS.md` M03.02/M15.02-M15.04/M19.01 for the specific open verification items.
Immediate next task family: none queued — owner to confirm the Dashboard card is gone and the `/listings` status tabs/counts render correctly; a future owner-approved production sync would let M03.02 graduate to `[x]`; variation apply/media/video remain separate, optional, owner-approval-only actions.
