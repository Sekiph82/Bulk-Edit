...
Immediate next task family: none queued — owner to open `/account/profile`, set first/last name, save, and confirm `/dashboard` greeting + sidebar cleanup + Account Sign out all work as expected; variation apply/media/video remain separate, optional, owner-approval-only actions.
