...
Immediate next task family: none queued — owner to click Sync on `/listings` when ready and report the exact result (this confirms or refutes the hotfix); the Phase B write-safety items need no owner action but are candidates for a future UI click-through pass; variation apply/media/video remain separate, optional, owner-approval-only actions.
