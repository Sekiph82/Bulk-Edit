...
**M08 deferred by owner decision.** The owner explicitly asked to defer M08 (owner/admin/beta management) until the customer-facing user modules — photo/media editor, video workflow, AI usage/suggestions, variations — are further along. No code change, no marker change (M08.04 stays `[~]`, M08.06 stays `[ ]`) — purely a scheduling/prioritization decision, recorded in `TASKS.md`'s M08 section header so it isn't mistaken for the active sprint in future sessions.

**M13 Media Safety & Video Workflow sprint** (branch `feature/m13-media-restore-video-workflow`). Preflight (reading `bulk_edit_media.py`, `etsy_media_write.py`, `video_generator.py`, `video_renderer.py`) found the media/video foundation was already far more mature than TASKS.md's own wording suggested: `add_image`/`replace_image`/`delete_image`/`add_video`/`replace_video`/`delete_video` were all fully implemented and callable via `POST /bulk-edit/media/jobs` + `.../apply`, with a real `ListingMediaBackupSnapshot` (rank/URL/alt-text per image) written before every destructive write — but **no restore endpoint existed anywhere**, and — more importantly — **no backend-level gate existed either**: the only thing stopping a destructive write from actually hitting live Etsy was a disabled option in the frontend dropdown. A direct API call, a script, or a future frontend bug could have triggered a real delete/replace with zero server-side protection.

Fixed both gaps. New `restore_images` media-job operation (`bulk_edit_media.py::restore_media_backup()`) re-uploads a backup's images in original rank order after deleting current ones — reuses the existing job/result/backup infrastructure entirely rather than inventing a parallel system. New `GET /bulk-edit/media/backups` (org-wide) and `POST /bulk-edit/media/backups/{id}/restore` (creates a pending job; the existing apply endpoint runs it). Idempotency via new `restored_at`/`restore_media_job_id` columns (migration `0028`), set only on real success so a job that's created but never applied doesn't block a genuine retry. Video restore explicitly not built — `videos_snapshot` only has an Etsy CDN URL, and Etsy's video-upload endpoint needs a local file, a separate download-and-stage task.

New `settings.MEDIA_DESTRUCTIVE_ACTIONS_ENABLED` (default `False`) now actually blocks every destructive operation — `replace_image`, `delete_image`, `replace_video`, `delete_video`, and the new `restore_images` (which itself deletes-then-reuploads, same risk) — at both job creation and apply time. `add_image`/`add_video` are never gated since nothing existing is ever destroyed by them. This flag stays off in production; only an owner decision can change that.

Frontend: new "Backups & Restore" section on `/media` (backup list, restore status, a Restore button that calls the real — currently 403ing — endpoint). Stale "coming soon, no restore yet" copy corrected everywhere to the truthful "disabled until restore is owner-verified," since restore genuinely exists now, the gap is verification, not the feature.

Video Generator: found the render pipeline was already fully real — `video_renderer.py` is local ffmpeg, no external AI/video provider anywhere, and it never calls Etsy (upload only ever happens through a separate, explicit media job). The actual gap was history: only the single in-progress render was ever tracked. New "Recent Videos" section lists every past render via a new `all_statuses=true` param on the existing `GET /video-generator/renders` (default stays completed-only, still used by the Replace Video picker in `/media`, unchanged).

Along the way, a pre-existing regression risk was found and fixed in the test suite: 46 existing `test_bulk_edit_media.py` tests exercised destructive operations directly against a mocked Etsy layer with no flag patched — the new gate would have broken them. Added one autouse fixture (`_enable_destructive_media_actions`) enabling the flag for that whole file's test scope, since those tests are about destructive-operation mechanics, not production safety (which the new gate-specific tests cover separately). Also fixed two real test-only bugs while writing the new restore tests: a schema-level `VALID_OPERATION_TYPES` duplicate set in `schemas/bulk_edit_media.py` that hadn't been updated for the new `restore_images` type (422 instead of the intended 403/201), and the established SQLAlchemy identity-map staleness pattern from earlier in this session (capture an id before `expire_all()`, or a post-write re-query returns a cached stale object).

16 new/updated backend tests. `tsc --noEmit`/`next lint`/`next build` all clean. M13.04/M13.05 both stay `[~]` — real, tested, no owner-live-run of destructive media or video generation this round.

**Safety:** no Etsy API call anywhere in this round; `MEDIA_DESTRUCTIVE_ACTIONS_ENABLED` stays `False`; no production sync, Apply, or Revert run by Claude/Codex; no live media write; no video generation; no Etsy video upload; no secrets printed.
...
**Fix:** new `build_expected_after_values()` in `bulk_edit_revert.py` resolves each changed field's expected-current value from sources tied to the specific apply job being reverted, in priority order: (1) the M06.04 per-field `AuditLog` row for that `apply_job_id` + listing + `result_status="success"` → `extra_data["after"]`; (2) for older jobs predating that audit trail, `BulkEditPreviewItem.diff[field]["after"]` for the same session+listing (the apply only ever wrote that value, and `revert_apply_job()` only considers listings whose apply result was itself `"success"`, so it's equally trustworthy). The local `Listing` row is never consulted as a value source — `detect_revert_conflict()`'s signature was changed from `(listing, snapshot_data, current_etsy_data, changed_fields)` to `(expected_after, changed_fields, current_etsy_data)`, removing the local-row parameter entirely so no future call site can accidentally reintroduce the bug. A changed field with no reliable source (old job, deleted preview item) is left out of `expected_after` and reported unverified — the whole item blocks with `unverified_fields: ["*"]` and the required copy: *"Cannot verify the expected post-apply value for this field, so automatic revert is blocked to avoid overwriting newer work."*

`test_bulk_edit_revert_conflict.py` rewritten to the new API: 21 tests (was 9). New coverage: `test_same_app_later_write_regression_blocks_revert_of_stale_job` (the exact audit scenario — applies Job A, then a real second bulk-edit session/apply as Job B on the same listing, then attempts reverting Job A with live Etsy mocked to match Job B's value; asserts the revert is refused and no Etsy PATCH is attempted), `build_expected_after_values()` unit tests for both sources plus the no-source case, `test_revert_blocked_when_old_job_has_no_captured_after_value` (end-to-end, audit rows + preview diff both deleted), `test_mixed_job_one_safe_one_conflict_partial_revert` (two listings, one clean revert proceeds, one conflicted item is skipped, exactly one Etsy write call made), and `test_unsupported_field_touched_blocks_revert_even_with_after_value` (having a real after-value doesn't bypass the missing-comparator rule for fields like `tags`).

No frontend change needed — the existing amber "conflict" `RevertResult` badge and per-row `error_message` display already surface the new/updated reason text correctly without modification.

**Checks:** `test_bulk_edit_revert_conflict.py` 21/21 passed. Targeted regression suite (`test_bulk_edit_revert.py`, `test_bulk_edit_inventory.py`, `test_write_audit_trail.py`, `test_job_states.py`, `test_bulk_edit_apply.py`) 153/159 passed, 6 failed — all 6 the established local-venv-only `requires_auth` 401-vs-403 artifact in unrelated auth-gate tests this round never touched. Full-suite run and frontend checks recorded in the execution log.

**Safety:** no Etsy write/status-mutation endpoint touched (the conflict check's own Etsy call stays GET-only, unchanged behavior from PR #121); no listing activation/deactivation/renewal/deletion; no production shop sync, Bulk Edit Apply, or Magic Revert executed by Claude/Codex; no secrets printed.
