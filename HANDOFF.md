...
**Current state: Etsy OAuth shop connection is fully working in production** (confirmed 2026-08-27 — WearYourStoriesCom, shop ID `44263504`, connected; issue #80 closed; see the `2026-08-27` "Etsy OAuth: production connection confirmed working" `CHANGELOG_AI.md` entry, and PR #84, `docs/etsy-oauth-success-handoff`, **still open, not yet merged** as of this write-up — check its status before assuming those doc updates are on `main`).

**New finding today (2026-08-28), NOT a bug:** owner ran the first production listing sync — Bulk Edit imported 25 listings, Etsy Shop Manager shows 210 active. Investigated as a possible pagination bug per the task's premise. **It isn't one** — `sync_shop_listings()`'s pagination loop is already correct; the cap is `PLAN_LIMITS["free"]["max_listings"] = 25`, a deliberate, already-tested feature gate, and the internal test account is confirmed on the Free plan. Implementing the requested "fix" would have meant bypassing that gate — against `CLAUDE.md` rule 8. Stopped, asked the owner, who chose: upgrade the test account's plan via the existing admin comp-grant path (`POST /admin/organizations/{org_id}/comp`, Owner Console → Organizations) rather than changing any code. **No code changed, no PR beyond this docs one, no new GitHub issue** (would have been factually wrong as scoped). Full detail: `CHANGELOG_AI.md` + `DECISIONS.md`, entries dated `2026-08-28`.

**Exact next step (owner action, not engineering):** grant the internal test account (or whichever org needs it) a comp plan via the Owner Console — this session has no superuser credentials to do it directly. After that, an owner-approved read-only re-sync should show ~210 listings instead of 25, confirming the plan cap (not pagination) was the whole explanation.
