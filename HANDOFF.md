...
**Checks:** `npx tsc --noEmit` clean, `npx next lint` no new warnings, `npx next build` clean (`/account/profile` route confirmed in output). Backend: full suite run — 1001 passed, 30 failed, all 30 matching the pre-existing `*_requires_auth`/`*_blocked_when_etsy_not_configured` 401-vs-403 baseline pattern (confirmed present on `main` before this branch too) plus 2 unrelated pre-existing `test_video_generator.py` failures — zero new failures from this round's changes; all 5 new `test_auth.py` profile tests pass. `git diff --check` clean, manual secret scan clean.
