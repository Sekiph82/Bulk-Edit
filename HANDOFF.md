...
## RESUME HERE — 2026-08-27 (sixth same-day update)

**Current state:** root cause of the Etsy OAuth failure is now known from real production logs: `etsy_oauth_shop_lookup_failed`, Etsy's `GET /v3/application/users/{user_id}/shops` returned **403** twice after token exchange succeeded. Doc research (`developers.etsy.com` + a generated mirror of Etsy's own OpenAPI spec) confirmed the app's `user_id` derivation (`access_token.split(".")[0]`) matches Etsy's documented `{numeric_user_id}.{opaque_token}` token format — so this is most likely an **access-tier restriction** on the app's Personal Use grant, not a code bug. GitHub issue `#80` tracks getting Etsy's answer on this. In the meantime, shipped defensive hardening (branch `fix/etsy-oauth-user-id-validation`, not yet merged as of this write-up): validates `user_id` is present and numeric *before* calling Etsy at all, new category `etsy_oauth_user_id_missing_or_invalid` — doesn't fix the 403, just closes a real gap (nothing previously stopped a malformed id from reaching Etsy as a raw request). Full detail: `CHANGELOG_AI.md`, two entries dated `2026-08-27` (safe logging, then this validation fix).

**Exact next step:** merge this PR (once CI green) → production redeploys → **do not** retry OAuth. The actual unblock is external: owner needs to get Etsy's answer on whether `GET /v3/application/users/{user_id}/shops` is available at the app's current Personal Use access level (message drafted during diagnosis, in the conversation that produced issue `#80`) — or test with an Etsy account confirmed to own an active shop, to rule that out separately. No further OAuth attempt should happen until one of those two things changes; another blind retry will almost certainly reproduce the same 403.
