...
## RESUME HERE — 2026-07-14, sixth session (retention cleanup: Option B → Option A, confirmed live)
...
8. **Local Postgres verification, real DB (not SQLite):** started Docker Desktop, brought up the local `postgres` container with the `docker-compose.dev-ports.yml` host-port override (port 45432, since 55432 hit the known Windows Hyper-V binding issue again — see prior `DECISIONS.md` entry), created an isolated `retention_verify` database, ran `alembic upgrade head` (reached `0025` cleanly), seeded exactly 4 expired + 4 unexpired rows (one pair per table). Dry-run reported exactly 4 candidates and a `ROLLBACK` (no writes); the real run deleted exactly 4 and a direct SQL count confirmed exactly the 4 unexpired rows remained; a second real run deleted 0. Dropped the verify DB and stopped the container afterward.
9. **Full verification pass:** backend full suite **982 passed** (975 baseline + 7 new), 0 failed. Targeted retention tests 7/7 standalone. `py_compile` clean, import validation clean, Alembic single-head confirmed `0025`. Frontend `tsc --noEmit` clean, `next lint` 0 errors (pre-existing warnings only), `next build` clean. `git diff --check` clean. Secret scan clean (only the standard test-fixture password `Test1234!` already used throughout the existing suite).
10. **Committed in the 3 specified logical groups** (`0184904` feat/dry-run+tests, `8cc1b5a` ops/app-spec, `4195dd0` docs), pushed, opened **PR #58**. All 6 required checks passed. Ran the pre-merge production checks (DB backup current, health OK, migration unaffected — no schema change in this PR) and a pre-merge production dry-run via direct read-only query (the new `--dry-run` code wasn't deployed yet, so queried the same aggregate directly): **0 expired candidates across all 4 tables.**
11. **Merged PR #58** (`5f0cdb8`, normal merge commit, non-force). Synced local `main`. Both prod apps auto-rebuilt from the new commit (existing components only — this alone does *not* add a new job component) and reached `ACTIVE`; health/DB/Redis/Private-Beta/migration-0025 all reconfirmed unaffected.
12. **Applied the new job component:** re-validated `ops/app-specs/bulk-edit-prod-api.yaml` against the live app one more time via `propose` (unchanged, still clean), then ran `doctl apps update bulk-edit-prod-api --spec ... --wait` — an additive-only change that preserves every existing service/env var/setting verbatim, adding only the one new job. Deployment `4f22a22f` reached `ACTIVE`; health reconfirmed again.
13. **Verified the job component directly against the live app** (narrow, filtered `doctl` query — no full spec dump): `name: retention-cleanup`, `kind: SCHEDULED`, `schedule.cron: "30 3 * * *"`, correct `run_command`, 1 instance, smallest size, absent from the `services`/`domains` lists (confirms no public route).
14. **Ran a final post-deploy production dry-run** (direct read-only query again): `0` expired candidates across all 4 tables — same as the pre-merge check, well below the anomaly thresholds (>10,000 total or >5,000 any single table). Per the owner's explicit instruction, did **not** manually trigger the real cleanup just to prove it works — the first actual execution is left to the 03:30 UTC scheduled run.
15. Docs flipped from "Option B, in progress" to "Option A, confirmed live" — via a small `docs/retention-scheduler-confirmed` branch/PR (main is branch-protected, same pattern as PR #57).

**Not yet done:** the first real scheduled execution (03:30 UTC) has not happened yet as of this session — not claimed as succeeded anywhere in the docs. Verify next session, or check `doctl apps logs bulk-edit-prod-api --component retention-cleanup --type run` once it has. The Etsy appeal draft (`ETSY_FINAL_APPEAL_DRAFT.md`) is the one remaining task from this session's brief — not yet written.

**Exact next step:** write `ETSY_FINAL_APPEAL_DRAFT.md` (do not submit it — owner approval required first). Separately, whenever convenient: confirm the first scheduled retention-cleanup run actually executed and succeeded.
