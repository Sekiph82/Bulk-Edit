...
**Current state:** Private Beta previously blocked sign-in entirely (every `APP_PREFIXES` app-route, including `/login`, redirected to `/private-beta`) — this also meant the Etsy OAuth callback's `/shops?connected=true`/`?error=...` redirect was silently masked to `/private-beta?...` before any user could see the real result (this is what a same-day live OAuth debug attempt hit: `/private-beta?error=etsy_connect_failed`, root cause traced to this masking, not to Etsy or the token exchange itself — see `DECISIONS.md` 2026-08-27). Fixed on branch `fix/private-beta-allow-signin` (off `main`, not yet merged as of this write-up — see PR status below/owner action): Private Beta now blocks only `/register`, `/signup`, `/get-started`; sign-in and the rest of the authenticated app pass through the middleware, and `/shops?connected=true`/`?error=...` reaches the real page. Full detail: `CHANGELOG_AI.md` entry `2026-08-27`.

**Exact next step:** merge `fix/private-beta-allow-signin` (CI green, per this task's own flow) → production auto-redeploys both frontend and backend (docs-only PRs before this one only redeployed because `deploy_on_push` has no path filter; this one has real frontend changes) → re-verify `/login` reachable and `/register` still paused on production → **do not** attempt live Etsy OAuth again until the owner explicitly asks (this fix only unblocks sign-in; the underlying OAuth attempt itself is a separate, still-pending owner-approved action).
