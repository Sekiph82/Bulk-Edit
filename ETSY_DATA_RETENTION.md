...
- **This branch adds:**
  ...
  - A manually-triggerable cleanup path (`scripts/run_retention_cleanup.py`) plus a documented cron/Celery-Beat hook in `docs/operations/WORKERS.md` for production scheduling (no Celery worker is deployed yet — see `PROJECT_STATUS.md` — so this ships as a script to be invoked by an external scheduler, consistent with how `scheduled_jobs` already works via a `run-due` endpoint pattern rather than a live worker).
