...
- **Kept at `[~]`, not `[x]`:** video generation itself (the actual ffmpeg render succeeding end-to-end against real listing photos) has never been owner-run — this round only added history/copy around an already-implemented pipeline. Etsy video upload (M13.03) remains separately blocked/not owner-tested.
...
M13 PARTIAL — M13.01/M13.02/M13.07 SHIPPED; M13.04 media restore infrastructure + a real backend destructive-action gate SHIPPED and tested (2026-08-31), destructive ops (replace/delete image/video, and restore itself) stay fully disabled — both in the UI and now enforced server-side via `MEDIA_DESTRUCTIVE_ACTIONS_ENABLED` — owner visually confirmed the disabled/truthful state in production, still `[~]` pending an actual live restore test; M13.05 render history + confirmed-local-and-never-auto-uploads copy SHIPPED, owner visually confirmed the manual-publish/no-auto-upload copy and synced-photo selection in production, still `[~]` pending an actual owner-run video generation; M13.03/M13.06 remain BLOCKED on external prerequisites (owner-approved live test / third-party developer apps).
