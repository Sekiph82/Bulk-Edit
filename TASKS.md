...
- **Kept at `[~]`, not `[x]`:** field coverage, search/filter, and export are now all real and tested — but no owner has yet looked at `/account/activity`'s new Audit Trail section, used a filter, or downloaded/inspected a CSV export in a browser. Promote to `[x]` once an owner confirms the records are readable, filters work, and the exported CSV opens cleanly with no secrets/raw JSON garbage.

M06 PARTIAL — M06.01/M06.02 PASS (owner-verified live); M06.03 conflict detection SHIPPED for title/description/sku/price/quantity (remediated 2026-08-31 to compare against the apply job's own captured after-value, not the local Listing row — see MAJOR bug note above), other fields explicitly unverified-not-assumed-safe; M06.04 per-item audit trail + CSV export + UI SHIPPED and tested, not yet owner-click-through-verified.
