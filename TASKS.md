...
M13 PARTIAL — M13.01/M13.02/M13.07 SHIPPED; M13.04 media restore infrastructure + a real backend destructive-action gate SHIPPED and tested (2026-08-31), destructive ops (replace/delete image/video, and restore itself) stay fully disabled — both in the UI and now enforced server-side via `MEDIA_DESTRUCTIVE_ACTIONS_ENABLED` — until an owner runs a live restore test; M13.05 render history + confirmed-local-and-never-auto-uploads copy SHIPPED (the video generation pipeline itself remains not owner-run); M13.03/M13.06 remain BLOCKED on external prerequisites (owner-approved live test / third-party developer apps).
