...
M03 PARTIAL — listing sync foundation PASS; shared `ListingPicker` SHIPPED and migrated into Media + Variations (M03.04); full-status read-only sync (M03.02) and listing status filters + real counts (M03.03) both implemented and test-verified this round, kept `[~]` pending a real (owner-approved) production sync / owner click-through; CSV Import/Export SHIPPED, previously untracked (M03.06, audit addition); Dynamic Pricing/Bulk Edit/Video Generator/Promote `ListingPicker` migration remains PLANNED.
...
M06 PARTIAL — M06.01/M06.02 PASS (owner-verified live); M06.03 conflict detection SHIPPED for title/description/sku/price/quantity, other fields explicitly unverified-not-assumed-safe; M06.04 per-item audit trail SHIPPED with full required-filter search, export not built.
