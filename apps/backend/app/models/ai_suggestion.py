...
class AISuggestion(Base, TimestampMixin):
    ...
    ai_session_id: Mapped[str] = mapped_column(Uuid(as_uuid=False), ForeignKey("ai_sessions.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    converted_to_session_id: Mapped[str | None] = mapped_column(Uuid(as_uuid=False), nullable=True)
