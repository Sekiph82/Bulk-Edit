...
class RevertResult(Base, TimestampMixin):
    ...
    revert_job_id: Mapped[str] = mapped_column(Uuid(as_uuid=False), ForeignKey("revert_jobs.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    backup_snapshot_id: Mapped[str | None] = mapped_column(Uuid(as_uuid=False), ForeignKey("listing_backup_snapshots.id", ondelete="SET NULL"), nullable=True)
