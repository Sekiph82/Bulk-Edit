...
**Current status (2026-07-16):** every FIXED item below is live in production. Item 22's retention cleanup is Option A (DO Scheduled Job); **second consecutive successful run confirmed 2026-07-16** (following the first on 2026-07-15) — see `ETSY_DATA_RETENTION.md` §2. Item 1's public-site alignment is now **complete after PR #64** (2026-07-16, merge `6be4046`) — remaining public AI wording neutralized and Privacy/Terms updated to match current production behavior; see `CHANGELOG.md`. Production DB and Redis connectivity, and the retention scheduler's config/cron/command, were all re-verified read-only the same day — API `/health`, `/health/db`, `/health/redis`, `/health/ready` all healthy. Items 8/9/19 (Etsy OAuth/write) remain BLOCKED — Etsy app still Banned, no reason given; **appeal has been submitted by the owner**, awaiting response. Items 6/29 remain MANUAL (owner-side, not re-verified this session — no new evidence either way).
...
1. **Homepage and public pages** — FIXED. Removed founding-access/pre-launch framing (see §"Website corrections" below); rest of the marketing site (features, pricing, FAQ, contact, blog, compare, tools) renders and builds cleanly. **Follow-up FIXED (2026-07-16, PR #64):** a later full forbidden-term audit found the earlier pass had left some public AI wording live (homepage hero "AI optimization", pricing "AI credits", `/features` metadata/safety line, FAQ, feature registry) — all neutralized, Privacy/Terms updated to match current AI-safeguard and retention/account-deletion behavior, and the whole site re-verified live after deploy.
