...
| `app.production-backend.yaml` | FastAPI + migrate job + PG + Redis | `api.bulkeditapp.com` | `main` | Live. Etsy configured (credentials issued 2026-07-31, set as encrypted `SECRET` env vars via `doctl apps update --spec`, not in this file); Stripe live since 2026-07-10; email configured via existing verified `bulkeditapp.com` Resend domain |
